...
def generate_comparision(dataset_id, algorithm_id, k_min, k_max, n_sim):
...
def generate_metrics_results(X, algorithm_id, response):
    ...
    for k in rng:
        response[algorithm_id][k] = {}
        for met in metrics:
            response[algorithm_id][k][met] = {}
        for n in range(response['n_sim']):
            ...
            for met in metrics:
                if met == 'silhouete':
                    response[algorithm_id][k][met][n] = Metrics.silhouette(
                        algorithm.clusters, len(X))
                elif met == 'calinskiHarabaszIndex':
                    response[algorithm_id][k][met][n] = Metrics.variance_based_ch(
                    ...
                    response[algorithm_id][k][met][n] = Metrics.gap_statistic(
                        clusters, random_clusters)

        for met in metrics:
            met_mean_std = []
            for n in response[algorithm_id][k][met]:
                met_mean_std.append(response[algorithm_id][k][met][n])
            response[algorithm_id][k][met]['mean'] = np.mean(met_mean_std)
            response[algorithm_id][k][met]['std'] = np.std(met_mean_std)
